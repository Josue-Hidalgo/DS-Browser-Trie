...
static void processLinePerLine(ifstream& file, Trie*& book, Dictionary<char, char>* validDict, Dictionary<char, char>* toLowerDict, Dictionary<int, string>*& linesList) {
	...
	while (getline(file, line)) {
		if (!line.empty() && linesList != nullptr) {
			...
			//cout << "Insertando: " << line << " en la l�nea: " << lineNumber << endl;
		}
		bool inword = false;
		for (char c : line) {
	...
	try {
		...
		while (option != 0) {
			...
			case 1:{
				...
				cout << "Resultados de la B�squeda: " << endl;
			...
			case 2:{
				...
				if (!book->containsWord(word))
					...
					cout << "Resultados de la B�squeda: " << endl;
					cout << "La palabra '" << word << "' sale " << linesList->getSize() << " veces y fue encontrada en las siguientes l�neas: " << endl;
			...
			default:
				cout << "Opci�n no v�lida. Por favor, ingrese un n�mero entre 1 y 5." << endl;
				break;
			}
			waitAndJump();
		}
