 ...
 * Descripci�n General:
 *
 * Este archivo contiene la implementaci�n de una clase MaxHeap, que es un
 * mont�culo m�ximo. Esta estructura de datos permite almacenar elementos de
 * valor num�rico y mantener la propiedad de mont�culo, donde el elemento
 * que se encuentra en la ra�z es el mayor de todos los elementos.
 *
 * Autor: Josue Hidalgo
 *
 */
...
public:
	...
	MaxHeap<E>& operator=(const MaxHeap<E>& other) {
		if (this != &other) {
			delete[] elements;  // liberar memoria previa
			max = other.max;
			size = other.size;
			elements = new E[max];
			for (int i = 0; i < size; ++i) {
